...
export function FocusScreen(props: FocusScreenProps) {
  ...
  const secondaryMain = view === 'analytics' ? <FocusAnalytics profile={profile} sessions={source.sessions} activityLog={source.activityLog} subjects={source.analyticsSubjects} />
      : view === 'friends' ? <FocusFriends username={profile.username} friends={source.friends} requests={source.requests} onSearchPeople={onSearchPeople} onUsernameChange={onUsernameChange} onCreateInviteLink={onCreateInviteLink} friendRequestsEnabled={preferences.friendRequests} onFriendRequestsChange={value => changePreference('friendRequests', value)} onAction={(action, personId, requestId) => onFriendAction?.(action, personId, requestId) ?? false} />
